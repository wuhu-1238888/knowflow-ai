...
**状态:2026-09-10 已完成(evaluation-report.md v1.1;检索层达标 + Δ 增益诚实呈现 + NFR 实测;DeepSeek 真实重跑完成)。** 检索层 hybrid_rerank Hit@5 12/12、MRR 0.9583 双批一致可复现;性能 NFR「检索 <3s」不达标(~31s/问)如实记录,截断实验负结果人拍板回退;评测全量 8 分 2 秒达标;真实 DeepSeek 重跑(Key 由人写 .env):132817Z 批次三模式 0 错配、拒答 2/2 且 0 误拒、生成 NFR 2.17–2.63s 达标;引用编号两轮缺陷修复(125001Z 3 处 → 130725Z 7/7/9 → 132817Z 0)全程带单测;剩余 = 五线人工判定(review-132817Z)+ C13/C14 冲突线口径拍板。
