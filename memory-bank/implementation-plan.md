...
**状态:2026-09-10 已完成(全量)。** 后端:runs 表迁移(status/per_case_json)+ update_run + 三端点(GET runs/GET runs/{id}/POST run,202/409/30 分钟过期恢复)+ 后台线程批次执行,pytest 149/149;前端:页头「检索评测」+ 动态矩阵(行=最新完成批次三模式,列=7 类场景(逐例明细推导)+Hit@5+MRR,混合+重排「当前默认」徽标,未实现指标「暂无数据」)+ 运行评测按钮(运行中禁用 + 按钮内 16px 细环 + 矩阵骨架脉动 + 5 秒轮询)+ RunList 运行历史(状态徽标/params/commit 截断/指标/展开逐例明细)+ 历史 CLI 批次诚实降级,前端 vitest 125/125、typecheck 干净;实机三模式真实评测通过(12/12·0.9583 / 12/12·0.9028 / 12/12·0.9583,与历史批次一致)。偏差记录见 progress.md Round 5 3.3.4 记录(①真实评测约 8 分钟;②BFF 将 202 归一为 200;③选型叙事去除硬编码数字;④矩阵只展示最新完成批次;⑤测试用未来时点 created_at 固定新鲜 running 判定)。L4 走查待人操作(qa-guide-3.3.4.md)。
...
**状态:2026-09-10 已完成(全量,零后端改动)。** 交付:关于页四区块(定位一句话与 product-vision 定稿逐字一致;synthetic 声明卡中性卡片 + 正文级排版;技术栈 6 行 mono 与仓库实况核对;边界说明 6 条摘要)。测试:page.test 8 例(含 grep 级断言无「真实企业客户」「真实商业项目」),前端 vitest 133/133、tsc 干净;实机 /about 200 关键内容齐。L4 走查待人行(qa-guide-3.3.5.md)。偏差记录无。
...
**状态:2026-09-10 已完成(链路实测 + 脚本定稿;页面级计时待人现场复核)。** 实测闭环逐段计时(真实模型经 BFF):打开 0.1s / 上传 61.1s 冷·4.9s 热 / 提问命中新文档 36.5s(3 引用全中,相关度 0.99)/ 来源核对逐字一致 / 拒答 C11 31.4s(no_answer=True);热态合计 ≈2.4 分钟、冷态最坏预算 ≈4.0 分钟,均 ≤ 5 分钟。摩擦点修正(零代码):准备阶段预热(计时外)消化模型首载、上传「解析中」徽标转正为讲解时间、演示后清理恢复 20 篇基线。产出物:demo-script.md v1.0 + docs/demo-data/upload/doc-cafe-01.md(合成演示上传文档)。测试基线不变(149/149 + 133/133)。
...
**状态:2026-09-10 已完成(evaluation-report.md v1.2;检索层达标 + Δ 增益诚实呈现 + NFR 实测;DeepSeek 真实重跑完成)。** 检索层 hybrid_rerank Hit@5 12/12、MRR 0.9583 双批一致可复现;性能 NFR「检索 <3s」不达标(~31s/问)如实记录,截断实验负结果人拍板回退;评测全量 8 分 2 秒达标;真实 DeepSeek 重跑(Key 由人写 .env):最终批次 140445Z 三模式 0 错配、拒答 2/2 且 0 误拒、结构化冲突 2/2 零误报(人拍板规则侧兜底)、生成 NFR 2.17–2.63s 达标;引用编号两轮缺陷修复(125001Z 3 处 → 130725Z 7/7/9 → 132817Z 0)全程带单测;剩余 = 五线人工判定(review-140445Z)。
...
**状态:2026-09-10 机器侧完成(progress.md 3.4.3 节),L4 页面人工走查待人执行。**

### 任务 3.4.4 重新生成交互优化(回答版本管理)
> 2026-09-10 人下达:重新生成不能再「平铺堆叠相同卡片」,须体现「当前可信答案 + 必要时查看历史版本」。
- 做什么:最新回答主卡(「AI 回答 · 最新」+ v{n} · 相对时间)+ 上一版回答默认折叠(展开后弱化版卡片,仅复制操作);重新生成卡内加载态(单请求单文案,不伪造多阶段);新旧内容一致 → 明确提示「已完成重新生成,本次回答与上一版一致」不复制卡片;连续重新生成 v1→v2→v3 序号连续(MVP 保留最新+上一版);冲突/引用随版本存储不串版本;拒答作为当前结果、原回答降级;失败恢复原版本;不动检索策略/评测页/文档库/全局布局,零新依赖
- 依赖:3.3.2、3.4.3
- 验证:L3 组件测试(vitest 新增 16 例,149/149)+ typecheck + L4 页面走查
- 产出物:page.tsx 版本状态机、PreviousAnswer 组件、AnswerSheet 版本/加载态、走查记录

**状态:2026-09-10 机器侧完成(vitest 149/149、typecheck 干净、SSR 200),L4 页面人工走查待人执行。**

### 任务 3.4.5 口径不一致 Trust UX 优化
> 2026-09-10 人下达:「口径不一致」不能是与回答平级的独立大卡,须作为当前 Answer 的 Trust/Evidence 层。
- 做什么:冲突提示内嵌 AnswerSheet(正文之后、依据与来源之前);默认轻量 Trust Alert「发现 N 份文档存在口径差异」+ 中立说明 + 「查看冲突来源」手风琴展开(再点收起);展开来源卡复用版本 citations 富化(标题/上传时间/「查看原文」→ 既有 SourceDrawer,无匹配 citation 降级为 doc_id 且无查看入口);不伪造冲突点抽取(仅展示双方原文摘录);小 warning 图标 + warning-bg 浅底(非红色错误样式);冲突随版本绑定(key=qa_id 隔离组件状态);无冲突/单来源不显示;拒答态不受影响
- 依赖:3.3.2、3.4.4
- 验证:L3 组件测试(vitest 新增 6 例,155/155)+ typecheck + L4 页面走查
- 产出物:conflict-panel.tsx 重做、AnswerSheet 冲突层、走查记录

**状态:2026-09-10 机器侧完成(vitest 155/155、typecheck 干净、SSR 200),L4 页面人工走查待人执行。**

### 任务 3.4.6 回答操作反馈(Feedback)
> 2026-09-10 人下达:点「有用/无用」页面几乎无变化、点「复制回答」零反馈,用户不知道操作是否成功。
- 做什么:有用/无用互斥选中态(brand-50 底 + 深紫字 + aria-pressed),重复点击同一项不重复提交,pending 禁用防重复;失败恢复原状态 + 轻量行内提示「反馈提交失败,请重试」(3s 自动消失,技术错误只进 console);复制回答基于真实 Clipboard 结果反馈「✓ 已复制」/「复制失败,请重试」1.8s 恢复,复制内容=回答原文不改契约;反馈按 Answer Version 的 qa_id 绑定(重新生成=新 qa_id 天然独立,零数据架构改动),挂载 GET /api/qa/{id}/feedback 恢复持久化状态(后端新增 GET 端点 + BFF GET + client getFeedback;404/未评价 → 未选中);上一版回答同样具备复制+反馈入口(不串版本);生成中无操作行 → 失效版本不可反馈
- 依赖:3.3.2、3.4.4
- 验证:L3 组件测试(vitest 新增 12 例,167/167)+ pytest 188/188 + typecheck + L4 页面走查
- 产出物:answer-sheet.tsx 操作行重做、后端 GET 反馈端点、走查记录

**状态:2026-09-10 机器侧完成(vitest 167/167、pytest 188/188、typecheck 干净、实机 ask→GET null→POST→GET useful 闭环),L4 页面人工走查待人执行。**
...
**状态:2026-09-11 机器侧完成(自检清单 11 条逐条执行,修复合计 7 项,详见 progress.md 3.5.1 节与 DesignRules 走查记录;375/768/1024 三断点与 prefers-reduced-motion 实景观感待人浏览器走查)。**
...
**状态:2026-09-11 机器侧完成(README 定稿:启动/模型/评测/synthetic/架构图/遗留说明齐全,CLI 命令逐一核对,seed 幂等实测;「从零实操」待人按 README 执行,详见 progress.md 3.5.2 节)。**
...
**状态:2026-09-11 M5 拍板通过(人拍板:全部达成)——L1–L5 全量回归绿、run JSON 归档核对一致、两条全局锚点全部达成、无新 Blocker;遗留 #1 解除,既有挂账项处置不变。Phase 3 实施收口(验收单 docs/m5-acceptance-checklist.md;Phase 4 按工作流另行执行)。**
