...
**状态:2026-09-10 已完成(evaluation-report.md v1.0;检索层达标 + Δ 增益诚实呈现 + NFR 实测;DeepSeek 真实调用实现;生成层五线真实验收待用户写 Key)。** 检索层 hybrid_rerank Hit@5 12/12、MRR 0.9583 双批一致可复现;性能 NFR「检索 <3s」不达标(~31s/问)如实记录,截断实验负结果人拍板回退;评测全量 8 分 2 秒达标;Mock 生成层结构事实入档;DeepSeekProvider 实现入库(b7527fc),待用户写 Key 后重跑 gen_eval 出真实核对表填五线;报告 137 项逐格自检全过,人工核对待人。
