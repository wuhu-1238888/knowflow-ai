...
**11. 生成中态**:检索中 = 灰阶 Skeleton;生成中 = 「AI 生成中」胶囊(唯一 pill + 唯一 shimmer 动画,渐变白名单第 3 处)。**除此处外,全站无任何加载动画装饰。**2026-09-14 问答页落地:后端无阶段状态 → 检索/生成合并为单一等待态(「AI 生成中」胶囊 + 答案区灰阶 Skeleton 同屏,不伪造「检索→重排→生成」分阶段);等待文案两档真实计时(20 秒后「仍在处理中,请稍候…已等待 N 秒」,不显示假百分比);提问按钮 loading = 浅蓝底 + 蓝字 + 按钮内 16px 细环(全站唯一允许的旋转加载图标)。
