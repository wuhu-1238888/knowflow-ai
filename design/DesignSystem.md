...
**11. 生成中态**:检索中 = 灰阶 Skeleton;生成中 = 「AI 生成中」胶囊(唯一 pill + 唯一 shimmer 动画,渐变白名单第 3 处)。**除此处外,全站无任何加载动画装饰。**2026-09-14 问答页落地:后端无阶段状态 → 检索/生成合并为单一等待态(「AI 生成中」胶囊 + 答案区灰阶 Skeleton 同屏,不伪造「检索→重排→生成」分阶段);等待文案两档真实计时(20 秒后「仍在处理中,请稍候…已等待 N 秒」,不显示假百分比);提问按钮 loading = 浅蓝底 + 蓝字 + 按钮内 16px 细环(全站唯一允许的旋转加载图标)。2026-09-14 精修(人规格):等待文案(胶囊 + 话术 + 冷启动提示)在骨架卡**顶部状态行内**(px-4,与真实回答卡元信息行同几何)——提示文字、Skeleton、最终回答卡属于同一个视觉内容容器;回答/拒答卡进场 = `{motion.answer-in}` 一次性动画(仅等待态 Skeleton → 结果切换触发;历史回看/快照恢复/重新生成不触发),禁缩放/弹性/渐变。
