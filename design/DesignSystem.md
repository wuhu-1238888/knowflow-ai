...
colors:
  # ── 品牌与 AI 强调色 ──
  brand-600: "#2563EB"          # 知流蓝:唯一品牌强调色(主按钮/引用/选中态/焦点态/AI 标识)
  brand-700: "#1D4ED8"          # hover
  brand-800: "#1E40AF"          # pressed / 深色文字变体(引用 chip 文字)
  brand-100: "#DBEAFE"          # 浅蓝底(选中项/引用 chip 底/证据高亮底)
  brand-50:  "#EFF6FF"          # 更浅蓝底(回答中引用句高亮/主浅蓝背景)
  focus-ring: "rgba(37,99,235,0.25)"   # 焦点环(2px)
  # ── AI 渐变(白名单:仅品牌标记/AI 眉题点/生成中胶囊) ──
  # 2026-09-11 品牌色紫→蓝:渐变两端均为 brand 深档→主档,白字对比度 ≥ 4.5:1,禁蓝紫混色
  ai-gradient: "linear-gradient(135deg, #1E40AF 0%, #2563EB 100%)"
  # ── 中性色 ──
...
spacing:
  ...
  s12: 48
containers:
  # ── 页面内容容器宽度(2026-09-09 人拍板:宽幅居中 + max-width 封顶;app-shell 按路由映射) ──
  content-qa: 960       # 知识问答:问题输入 + 回答阅读列(900–1000 档)
  content-wide: 1152    # 评测 / 文档库:指标矩阵与文档表格(1100–1200 档)
  content-reading: 960  # 知识问答 / 关于:阅读型页面(900–1000 档)
rounded:
  xs: 4       # 内联代码
...
motion:
  ...
  shimmer: "1.6s linear infinite"  # 仅「AI 生成中」胶囊呼吸动画(2026-09-14 人拍板退役:等待区改纯文字提示,token 保留备用)
  answer-in: "240ms ease-out both"  # 回答/拒答卡进场(2026-09-14 人规格):仅等待态 Skeleton → 结果一次性切换,opacity 0→1 + translateY 4px→0,禁缩放/弹性
components:
  # ── 按钮 ──
...
- **色彩人格**:蓝(`{colors.brand-600}`)是唯一的品牌强调色,意为「AI 的确定性、证据的可追溯」;它出现的地方只有四类——主按钮(提问/上传/运行评测)、引用(可点击的证据入口)、选中态(✓ 有用 = 浅蓝底 + 蓝字)、焦点态。除此之外整屏保持中性灰,让知识内容自己说话;冲突用琥珀、成功用绿、错误用红、禁用用灰(详见「色彩语义」职责表)。
...
> 2026-09-11 人拍板:品牌色由紫转蓝,产品定位「Blue Enterprise Knowledge Product」。颜色是产品语义,不是装饰;普通操作默认中性,只有「主行动 / 可追溯证据 / 已选中 / 聚焦」才用品牌蓝。

### 颜色职责表(新页面/新组件必须遵守)

| 色系 | 用于(允许) | 不用于(禁止) |
| --- | --- | --- |
| **蓝(brand-600 `#2563EB` / 700 `#1D4ED8` / 800 `#1E40AF` / 100 `#DBEAFE` / 50 `#EFF6FF`)** | ① 主按钮(提问/上传/运行评测);② 引用 chip 与依据编号([n] = 可点击的证据入口,浅蓝底 + 深蓝字,悬停反色);③ 选中态(✓ 有用/无用 = 浅蓝底 + 蓝字图标);④ 焦点态(焦点环/输入框描边);⑤ AI 身份(AI 眉题 8px 点、生成中胶囊、品牌标记);⑥ 链接 | 大面积蓝块、全蓝 Badge、普通操作按钮(重新生成/复制/查看原文)、导航整块深蓝、蓝紫混色渐变、发光、营销式蓝底 |
| **中性(surface/canvas/hairline/ink 系)** | 页面与卡片底、发丝线边框、正文与次级文字、普通按钮默认态(白/透明底 + 中性灰边框 + 深灰文字)、普通 Badge/Tag(如「混合」来源徽标之外的信息标签)、导航默认态、拒答卡、禁用底 | 传达「成功/失败/冲突/选中」——这些必须用语义色,不用灰 |
| **琥珀(warning/warning-bg/warning-border)** | **仅**文档口径冲突/分歧:冲突面板边框、「口径不一致」徽标 | 系统错误、删除确认(那些用红);品牌蓝(冲突 ≠ 品牌主张) |
| **绿(success/success-text/success-bg)** | 成功/完成:文档「已索引」、评测「达标」、「已复制」成功态 | 普通操作默认态、品牌表达 |
| **红(danger/danger-text/danger-bg)** | 真正的错误/破坏性:解析失败、删除确认、API/解析失败提示 | 拒答卡(拒答是正确行为)、冲突面板(分歧待用户裁决)、一般性「未命中」 |
| **灰(ink-disabled/surface-2 禁用底)** | 禁用/不活跃:禁用按钮、占位符、引用缺失 chip | 表达「有信息」(次级信息用 ink-3,不用禁用灰) |
...
| Token | 含义 | 出现位置 |
| --- | --- | --- |
| `{colors.brand-600}` | 品牌强调 / 可追溯证据 / 主行动 | 主按钮、链接、焦点态、AI 眉题点、引用 chip 反色态 |
| `{colors.ai-gradient}` | **AI 在场**。蓝色渐变仅用于 AI/品牌身份标识(白名单 3 处) | 品牌标记、AI 眉题 8px 圆点、「AI 生成中」胶囊 |
| `{colors.canvas}` / `{colors.surface}` | 页底灰 / 卡片白:用表面层次而非阴影建立层级 | 全部页面 |
| `{colors.hairline}` | 边界:1px 发丝线承担绝大多数分隔 | 卡片、表格、列表 |
| `{colors.success*}` | 文档「已索引」、评测「达标」、复制成功 | 状态徽标、指标表、成功态 |
...
│ (白底/右发丝线) │  容器居中,宽度按页分档(见下)    │ (点引用滑出,遮罩)  │
│               │                                 │                 │
...
- 页面容器宽度(`containers` token,2026-09-09 人拍板「宽幅居中 + max-width 封顶」,2026-09-14 关于页并入阅读型 960 档——两档体系:阅读型 960 / 数据型 1152;app-shell 按首段路由映射,未登记路由回落 content-qa):知识问答 960px(问题输入 + 回答阅读列);关于 960px(阅读型);评测 1152px(指标矩阵);文档库 1152px(文档表格)。全部 `margin: 0 auto` 居中,响应式 padding `px-4 sm:px-6`,1920px 下由 max-width 封顶、不无限拉伸;页内区块(页头/输入区/卡片/表格)共用同一容器宽度,禁止区块级另设 max-width。
- 源文档正文阅读列宽与问答页容器一致(960px 内容列,正文 16px/1.7 不压缩)。
...
- 滚动模型(2026-09-14 人规格):侧栏 `fixed inset-y-0 left-0`(224px)固定于视口左侧,自身 `overflow-y-auto`——内容超高(小视口高度)时仅侧栏内部滚动;主内容随 document 原生滚动(无独立滚动容器),内容列 `lg:pl-56` 让位固定侧栏——无重叠、无横向溢出、无双滚动条;路由切换自动回顶与返回恢复滚动位置沿用浏览器原生行为;`<1024px` 侧栏收起,左内边距归零。
- 页面头节奏:页头区(标题 + 操作)高度 64px,与正文间距 `{spacing.s6}`;页内分区间距 `{spacing.s8}`。可选副标题:标题下一行 body-sm ink-2 单行(仅问答页:「企业知识助手,基于企业知识库回答问题」)——克制,禁营销文案。
- 问答页垂直节奏(2026-09-09 人拍板,页面级):主内容顶部呼吸空间 24px(<1024px)/48px(≥1024px);页头 → 提问区 24px;各状态区块间统一 24px,加载/空态/回答/拒答/错误同位不跳动。其他页维持现状。
...
**8. 上传区(UploadZone)**:发丝虚线框卡(圆角 10px、左右内边距 32px、上下 24px,整体高约 160px);信息层级 = 线性 Upload 图标(20px,中性灰)→ 主标题「拖入或选择文档」(16/600)→ 支持格式「支持 .md / .pdf / .docx / .html / .txt」(body-sm,扩展名 mono)→ caption「不支持扫描件 OCR,请先将扫描件转换为文本」(2026-09-14 人规格,当日回调收紧:操作提示行「点击选择文件…」删除、图标 24px→20px、上下内边距收至 s6)。状态:Default 中性灰虚线 + 白底;Hover 虚线变品牌蓝 + brand-50 浅蓝底 + 图标轻染蓝(仅 150ms 色彩过渡);Drag Over 品牌蓝实线 + brand-50 底 + **支持格式行**原位换「松开鼠标以上传」(不增行,无布局跳动;dragenter/dragleave 深度计数防抖动);Error danger 虚线 + danger-bg 底 + 卡内 role=alert 错误行;Uploading 标题换「正在上传 {name}…」+ 支持格式行原位换「上传中,请稍候…」+ 全通道禁用,无假进度动画(真实进度 = 表格乐观行)。键盘 Enter/Space 触发,焦点环走全局 :focus-visible。
...
**11. 生成中态**:检索中 = 灰阶 Skeleton;生成中 = 等待区顶部唯一文字提示(2026-09-14 人拍板定稿:原「AI 生成中」胶囊自 UI 退役——等待态 = 骨架卡**上方**纯文字提示,左对齐排版、与卡片左边缘对齐,卡内只有骨架屏,无胶囊无图标;shimmer/pill token 保留备用)。**除此处外,全站无任何加载动画装饰。**2026-09-14 问答页落地:后端无阶段状态 → 检索/生成合并为单一等待态(文字提示 + 答案区灰阶 Skeleton 同屏,不伪造「检索→重排→生成」分阶段);等待文案两档真实计时(20 秒后「仍在处理中,请稍候…已等待 N 秒」,不显示假百分比);提问按钮 loading = 浅蓝底 + 蓝字 + 按钮内 16px 细环(全站唯一允许的旋转加载图标)。回答/拒答卡进场 = `{motion.answer-in}` 一次性动画(仅等待态 Skeleton → 结果切换触发;历史回看/快照恢复/重新生成不触发),禁缩放/弹性/渐变。
...
```
空闲(提问框+示例问题) → 等待中(生成中胶囊 + 答案区 Skeleton 同屏;后端无阶段
  状态,不细分检索/生成;20 秒后文案转「仍在处理中…已等待 N 秒」真实计时) →
