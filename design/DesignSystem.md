...
containers:
  ...
  content-reading: 880  # 关于:阅读型页面(800–900 档)
rounded:
  xs: 4       # 内联代码
...
- 页面容器宽度(`containers` token,2026-09-09 人拍板「宽幅居中 + max-width 封顶」;app-shell 按首段路由映射,未登记路由回落 content-qa):知识问答 960px(问题输入 + 回答阅读列);评测 1152px(指标矩阵);文档库 1152px(文档表格);关于 880px(阅读型)。全部 `margin: 0 auto` 居中,响应式 padding `px-4 sm:px-6`,1920px 下由 max-width 封顶、不无限拉伸;页内区块(页头/输入区/卡片/表格)共用同一容器宽度,禁止区块级另设 max-width。
