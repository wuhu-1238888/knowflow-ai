/* 等待态 Skeleton(2026-09-14):模拟真实回答卡结构——状态行 → 正文
   ...
   2026-09-14 精修:等待文案(胶囊 + 话术 + 冷启动提示)移入卡片顶部
   状态行(px-4,与真实回答卡元信息行同几何)——提示文字、Skeleton、
   最终回答卡属于同一个视觉内容容器,文案不再漂浮在卡片上方。 */
...
export function AnswerSkeleton({
  ...
  /** 等待话术(两档真实计时,由页面状态机传入)。 */
  statusText: string;
  /** 本会话首问冷启动提示(仅长等待时传入,绝不常显)。 */
  ...
  return (
    ...
    >
      {/* 状态行:与真实回答卡的元信息行同位(px-4 py-3 border-b) */}
      <div className="border-b border-hairline px-4 py-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <p className="text-body-sm text-ink-2">{statusText}</p>
        </div>
        {coldStartHint ? (
          <p className="mt-1.5 text-caption text-ink-3">{coldStartHint}</p>
        ) : null}
      </div>
