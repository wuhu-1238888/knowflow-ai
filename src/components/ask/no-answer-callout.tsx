...
/* 拒答卡(DesignRules):中性灰 + 发丝线 + 信息图标;
   ...
   (保留在 API 响应与 QA 日志,评测页承载检索质量展示)。
   拒答两态(2026-09-13 闭环优化):no_evidence = 完全无依据(「依据 0 条」);
   insufficient = 检索到部分相关内容但不足以确定答案(明示条数,不强行生成)。 */

export interface NoAnswerCalloutProps {
  /** 后端 refusal_reason:insufficient = 有低分相关命中。 */
  reason?: "no_evidence" | "insufficient";
  /** insufficient 时检索到的相关信息条数(条数非分数,不进阈值口径)。 */
  relevantHits?: number;
}
...
export function NoAnswerCallout({
  reason = "no_evidence",
  relevantHits = 0,
}: NoAnswerCalloutProps) {
  const insufficient = reason === "insufficient";
  return (
    <section
      aria-label={insufficient ? "信息不足" : "未找到答案"}
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-heading-2 font-semibold text-ink">
          {insufficient ? "找到相关信息,但不足以确定答案" : "知识库中未找到答案"}
        </h2>
      </div>
      <p className="text-body-md text-ink-2">
        {insufficient
          ? "知识库中有部分相关内容,但不足以形成可靠回答,KnowFlow 不会强行生成确定性答案。"
          : "当前知识库中没有足够相关的内容来回答这个问题。"}
      </p>
      <p className="text-numeric text-ink-3">
        {insufficient ? `找到相关信息 ${relevantHits} 条,置信度不足` : "依据 0 条"}
      </p>
